...
// Default flag values
var defaultFlagValues = map[string]interface{}{
	"gitops-repo-url":        "",
...
func initiateInteractiveMode(io *BootstrapParameters, client *utility.Client, cmd *cobra.Command) error {
	...
	promptForAll := !ui.UseDefaultValues(defaultFlagValues)
	if io.GitOpsRepoURL == "" {
...
func NewCmdBootstrap(name, fullName string) *cobra.Command {
	...
	bootstrapCmd.Flags().StringVar(&o.GitOpsRepoURL, "gitops-repo-url", defaultFlagValues["gitops-repo-url"].(string), "Provide the URL for your GitOps repository e.g. https://github.com/organisation/repository.git")
	bootstrapCmd.Flags().StringVar(&o.GitOpsWebhookSecret, "gitops-webhook-secret", defaultFlagValues["gitops-webhook-secret"].(string), "Provide a secret that we can use to authenticate incoming hooks from your Git hosting service for the GitOps repository. (if not provided, it will be auto-generated)")
	bootstrapCmd.Flags().StringVar(&o.OutputPath, "output", defaultFlagValues["output"].(string), "Path to write GitOps resources")
	bootstrapCmd.Flags().StringVarP(&o.Prefix, "prefix", defaultFlagValues["prefix"].(string), "", "Add a prefix to the environment names(Dev, stage,prod,cicd etc.) to distinguish and identify individual environments")
	bootstrapCmd.Flags().StringVar(&o.DockerConfigJSONFilename, "dockercfgjson", defaultFlagValues["dockercfgjson"].(string), "Filepath to config.json which authenticates the image push to the desired image registry ")
	bootstrapCmd.Flags().StringVar(&o.ImageRepo, "image-repo", defaultFlagValues["image-repo"].(string), "Image repository of the form <registry>/<username>/<repository> or <project>/<app> which is used to push newly built images")
	bootstrapCmd.Flags().StringVar(&o.GitHostAccessToken, "git-host-access-token", defaultFlagValues["git-host-access-token"].(string), "Used to authenticate repository clones. Access token is encrypted and stored on local file system by keyring, will be updated/reused.")
	bootstrapCmd.Flags().BoolVar(&o.Overwrite, "overwrite", defaultFlagValues["overwrite"].(bool), "Overwrites previously existing GitOps configuration (if any) on the local filesystem")
	bootstrapCmd.Flags().StringVar(&o.ServiceRepoURL, "service-repo-url", defaultFlagValues["service-repo-url"].(string), "Provide the URL for your Service repository e.g. https://github.com/organisation/service.git")
	bootstrapCmd.Flags().StringVar(&o.ServiceWebhookSecret, "service-webhook-secret", defaultFlagValues["service-webhook-secret"].(string), "Provide a secret that we can use to authenticate incoming hooks from your Git hosting service for the Service repository. (if not provided, it will be auto-generated)")
	bootstrapCmd.Flags().BoolVar(&o.SaveTokenKeyRing, "save-token-keyring", defaultFlagValues["save-token-keyring"].(bool), "Explicitly pass this flag to update the git-host-access-token in the keyring on your local machine")
	bootstrapCmd.Flags().StringVar(&o.PrivateRepoDriver, "private-repo-driver", defaultFlagValues["private-repo-driver"].(string), "If your Git repositories are on a custom domain, please indicate which driver to use github or gitlab")
	bootstrapCmd.Flags().BoolVar(&o.PushToGit, "push-to-git", defaultFlagValues["push-to-git"].(bool), "If true, automatically creates and populates the gitops-repo-url with the generated resources")
	bootstrapCmd.Flags().BoolVar(&o.Interactive, "interactive", defaultFlagValues["interactive"].(bool), "If true, enable prompting for most options if not already specified on the command line")
	return bootstrapCmd
}
